...
pareto_front <- function(xi, yi, zi, wi, pareto_fronts = 1, sort = TRUE) {
  ...
  Dtemp <- D <- d[order(d$yi, d$xi, d$zi, d$wi, decreasing = FALSE), ]
  df <- data.frame()
  i <- 1
  while (nrow(Dtemp) >= 1 & i <= max(pareto_fronts)) {
    these <- Dtemp[which(!duplicated(cummin(Dtemp$xi)) & !duplicated(cummin(Dtemp$zi)) & !duplicated(cummin(Dtemp$wi))), ]
